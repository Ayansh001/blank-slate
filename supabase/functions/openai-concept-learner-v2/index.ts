...
serve(async (req) => {
  ...
  try {
    ...
    // Enhanced prompt structure matching Gemini's comprehensive approach
...
Generate the comprehensive learning package now:`;
    ...
    try {
      // Clean the content to extract JSON (like Gemini function)
      ...
      // Ensure flashcardSummaries exist (matching Gemini fallback)
      ...
      // Fetch YouTube videos using the search query (matching Gemini integration)
      let youtubeVideos = [];
      try {
      ...
      // Add YouTube videos to the result (matching Gemini structure)
      const enhancedResult = {
        ...parsedResult,
        youtubeVideos: youtubeVideos
      };
      
      // Save to concept learning sessions (matching Gemini approach)
      await supabase
      ...
      // Fallback: create structured response from plain text (matching Gemini fallback)
